...
Before(async function () {
  // Her senaryo yeni bir World nesnesi ve bağımsız bağlam ile başlar.
  this.scenarioState = {};
  await this.start();
});
...
After(async function ({ result, pickle }) {
  ...
  if (failed && this.page && !this.page.isClosed()) {
    try {
      const screenshot = await this.page.screenshot({ fullPage: true, timeout: 10_000 });
      await this.attach(screenshot, 'image/png');
    } catch (_) {
      // Artifact failure must not hide the original scenario failure.
    }
  }
  ...
  if (failed && video) {
    try {
      ...
      // Video may be unavailable when browser startup fails.
    }
  }
});
